...
> 사과 격자에서 합이 10인 직사각형을 골라 제거하는 게임. 이론상 최대 170점.
>
> **단, 보드마다 도달 가능한 최대 점수가 다르다.** 어떤 보드는 천장이 100점,
> 어떤 보드는 160점. 사이트의 rejection sampling은 "보드 합이 10의 배수"만
> 보장하지 "풀이 가능성"은 보장 안 함.
>
...
**Step 4는 실제 사이트 자동 플레이로 완료** (`scripts/auto_player.py`).

### 왜 학습이 단순 휴리스틱을 못 넘었나

이 결과는 "RL을 덜 돌려서 실패"라기보다, 이 게임의 성격상 꽤 자연스러운
실패에 가깝다. `greedy_smallest`는 단순해 보이지만 "작은 영역부터 제거해서
보드 구조를 최대한 보존한다"는 강한 도메인 지식을 담고 있다. 반면 PPO나
PointerNet은 그 규칙을 보상 신호만 보고 다시 발견해야 한다.

문제는 좋은 수의 효과가 바로 드러나지 않는다는 점이다. 지금 선택한 직사각형이
30~50수 뒤 최종 점수에 어떤 영향을 줬는지 신호가 희미하다. 실제 로그에서도
value head는 보드의 대략적인 점수 기대값을 잘 맞히는데(`explained_variance`
0.9+), policy는 valid 액션 사이에서 무엇을 골라야 하는지 배우지 못했다.

따라서 학습을 더 오래 돌린다고 `greedy_smallest`를 안정적으로 넘을 가능성은
낮다. 넘기려면 "처음부터 정책을 학습"하기보다 `greedy_smallest`를 기반 정책으로
두고, 예외 상황에서만 rollout/search/value 평가로 더 나은 수를 고르는 쪽이 더
현실적이다. 즉 이 프로젝트의 결론은 "학습이 항상 단순 알고리즘을 이긴다"가
아니라, **단순 휴리스틱이 담고 있는 도메인 지식을 학습 모델이 충분히 재발견할
수 있을 때만 이긴다**는 쪽에 가깝다.
