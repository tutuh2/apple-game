...
### 핵심 인사이트 -1: RL은 greedy_smallest를 재발견하고 있었을지도 모름
모든 RL 시도가 113 근처에서 정체. 만약 완전히 다른 정책을 찾았다면 점수가
크게 위/아래로 갔어야 함. 113 근처에서 막힌다는 건 **같은 결론에 수렴 중**
이라는 강한 신호.

가능성:
- (A) 진짜 재발견: 학습 끝에 greedy_smallest와 거의 같은 정책 → "실패"가 아님
- (B) 부분 재발견: 그 방향으로 가다가 신호 약해서 110에서 멈춤
- (C) 우연히 같은 점수: 다른 정책인데 점수만 비슷

검증 안 됨. 30분짜리 실험으로 확인 가능:
같은 시드 보드에서 RL 액션 vs greedy_smallest 액션 비교 → 일치율 측정.
일치율 높으면 (A) 또는 (B), 낮으면 (C).

이 가설이 맞다면 **5번의 RL 실패는 실패가 아니라**, 이 게임 자체가
RL이 발견할 수 있는 더 좋은 정책을 허용 안 하는 구조라는 증명에 가까움.
deterministic + perfect info + 단일 플레이어라서 RL의 장점(탐색)이
잘 안 살아남.

### 핵심 인사이트 0: "평균 N점"의 함정 (가장 늦게 알게 된 사실)
**보드마다 도달 가능한 최대 점수가 다르다.** 어떤 보드는 천장이 100, 어떤
보드는 160. 사이트 rejection sampling은 "보드 합 % 10 == 0"만 보장하지
"풀이 가능성"은 보장 안 함.

따라서 "사람이 130점대 낸다"는 건 **운 좋은 한 게임**의 얘기지 100판 평균이
아님. greedy_smallest 113.5 = 사람 평균과 큰 차이 없을 가능성이 높음.
RL이 113 못 깬 게 "실패"가 아니라 **이미 자기 천장 근처에 도달**한 것일 수도.

올바른 평가법은:
1. 같은 시드의 보드에서 정책 vs 사람 비교 (intrinsic ceiling이 같은 상황)
2. brute force/exhaustive search로 각 보드의 oracle 최대 구해서 갭 측정
3. "평균 +5점" 같은 작은 개선도 의미 있는 영역

이 사실을 모르고 "RL로 140점 평균 노리자"가 처음부터 비현실적 목표였음.

### 핵심 인사이트 1: 순서 의존성 (path dependency)
